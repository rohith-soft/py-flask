...
todo_schema = TodoModel.TodoSchema()
...
class TodoView(FlaskView):
    ...
    def get_all_todos(self):
        todos = TodoModel.TodoModel.get_all_todos()
        result = todo_schema.dump(todos, many=True)
        return {"todos": result}

    @route('/<int:todo_id>', methods=['GET'])
    def get_todo_by_id(self, todo_id):
        return todo_schema.dump(TodoModel.TodoModel.get_todo_by_id(todo_id))

    @route('/<int:todo_id>', methods=['DELETE'])
    def delete_todo_by_id(self, todo_id):
        try:
            todo = TodoModel.TodoModel.get_todo_by_id(todo_id)
            TodoModel.TodoModel.delete(todo)
            return "Deleted TODO"
    ...
    def update_todo_by_id(self, todo_id):
        try:
            ...
            todo = TodoModel.TodoModel.get_todo_by_id(todo_id)
            TodoModel.TodoModel.update(todo, request_data)
    ...
    def add_todo(self):
        try:
            request_data = request.get_json()
            todo = TodoModel.TodoModel(request_data['content'], request_data['completed'])
            TodoModel.TodoModel.create(todo)
